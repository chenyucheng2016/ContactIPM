...
struct CandidateEvaluator : TrialPointEvaluator<2, 1, 5> {
    ...
    int trial_candidate_count() const override { return 2; }
    bool evaluate(double alpha, double& out_theta, double& out_phi) override {
        return evaluate_candidate(0, alpha, out_theta, out_phi);
    }
    bool evaluate_candidate(int candidate, double, double& out_theta,
                            double& out_phi) override {
        out_theta = theta[candidate];
        out_phi = phi[candidate];
        return true;
    }
    double current_theta() const override { return theta0; }
    double current_phi() const override { return phi0; }
    double compute_Dphi() const override { return dphi; }
};

void test_candidate_selection() {
    TEST("Filter candidate phase-aware selection");

    FilterLSParams flp;
    flp.theta_min = 0.1;
    flp.soc_max = 0;

    // Both candidates pass the same filter test. The alternative dominates in
    // both filter coordinates, so it replaces the ordinary trial.
    CandidateEvaluator feas;
    feas.theta0 = 1.0; feas.phi0 = 10.0; feas.dphi = -1.0;
    feas.theta[0] = 0.5; feas.theta[1] = 0.1;
    feas.phi[0] = 5.0; feas.phi[1] = 4.0;
    FilterLineSearch<2, 1, 5> fls_feas;
    fls_feas.init(flp);
    LSResult result = fls_feas.search(feas, 1.0);
    if (result.status != LSStatus::ACCEPTED || result.candidate != 1
        || std::fabs(result.theta_trial - 0.1) > 1e-14) {
        FAIL("dominating candidate was not selected"); return;
    }

    // During feasibility restoration, an admissible lower-theta retraction
    // wins even when its objective is higher. The filter already bounds the
    // permitted tradeoff.
    CandidateEvaluator tradeoff = feas;
    tradeoff.phi[1] = 20.0;
    FilterLineSearch<2, 1, 5> fls_tradeoff;
    fls_tradeoff.init(flp);
    result = fls_tradeoff.search(tradeoff, 1.0);
    if (result.status != LSStatus::ACCEPTED || result.candidate != 1) {
        FAIL("feasibility phase did not choose lower theta"); return;
    }

    // Optimality phase: both candidates satisfy Armijo and theta control, so
    // choose the lower barrier objective.
    CandidateEvaluator opt;
    opt.theta0 = 0.0; opt.phi0 = 10.0; opt.dphi = -1.0;
    opt.theta[0] = 0.0; opt.theta[1] = 0.0;
    opt.phi[0] = 9.0; opt.phi[1] = 8.0;
    FilterLineSearch<2, 1, 5> fls_opt;
    fls_opt.init(flp);
    result = fls_opt.search(opt, 1.0);
    if (result.status != LSStatus::ACCEPTED || result.candidate != 1
        || std::fabs(result.phi_trial - 8.0) > 1e-14) {
        FAIL("optimality phase did not choose lower phi"); return;
    }
