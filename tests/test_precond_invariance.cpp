...
void test_single_iter_invariance() {
    TEST("Single-iteration step invariance (1e-10)");
    ...
    // Physical directions must agree tightly after undoing the diagonal
    // coordinate transform. This protects contact and non-contact systems
    // from scale-dependent Newton steps.
    double tol = 1e-10;
...
void test_contact_fallback_and_horizon_scaling() {
    TEST("Persistent-contact fallback and intermittent identity scaling");

    constexpr int TX = 1;
    constexpr int TU = 3;
    constexpr int TC = 2;
    constexpr int TN = 2;
    StageData<TX, TU, TC> stages[TN + 1]{};

    for (int k = 0; k <= TN; ++k)
        stages[k].Qxx(0, 0) = k == TN ? 9.0 : 0.0;
    for (int k = 0; k < TN; ++k) {
        stages[k].Quu(0, 0) = 4.0;
        stages[k].s[0] = stages[k].s[1] = k == 0 ? 0.5 : 0.25;
        stages[k].lambda[0] = stages[k].lambda[1] = k == 0 ? 2.0 : 1.0;
        stages[k].Cu(0, 1) = k == 0 ? 1.0 : 3.0;
        stages[k].Cu(1, 2) = 5.0;
    }
    stages[0].B(0, 1) = stages[1].B(0, 1) = 1.0;
    stages[0].B(0, 2) = 1.0;

    HessianPreconditioner<TX, TU, TN> prec;
    prec.compute(stages);

    double error = 0.0;
    for (int k = 0; k <= TN; ++k)
        error = std::max(error, std::fabs(prec.Lx(k)[0] - 3.0));
    for (int k = 0; k < TN; ++k) {
        error = std::max(error, std::fabs(prec.Lu(k)[0] - 2.0));
        error = std::max(error, std::fabs(prec.Lu(k)[1] - 6.0));
        error = std::max(error, std::fabs(prec.Lu(k)[2] - 1.0));
    }

    printf("(max_error=%.2e, persistent_contact=%.1f, intermittent=%.1f) ",
           error, prec.Lu(0)[1], prec.Lu(0)[2]);
    if (error > 1e-14) {
        FAIL("persistent contact fallback or intermittent identity missing");
        return;
    }
    PASS();
}
...
int main() {
    ...
    printf("  Single-iter tolerance: 1e-10 (one linear solve)\n\n");
    ...
    test_contact_fallback_and_horizon_scaling();
