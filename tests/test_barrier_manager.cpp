...
void test_ftb_and_accepted_step_are_separate() {
    TEST("FTB and accepted step have separate barrier roles");
    ...
    double mu = 1.0;
    for (int i = 0; i < p.restoration_count_limit; ++i) {
        strat.update(mu, 1e-10, 1e-10, 0.9, -1.0, true,
                     1.0, 0.0, 1e-3);
    }
    if (strat.restoration_active()) {
        FAIL("tiny accepted steps activated restoration without a violated constraint");
        return;
    }

    strat.reset(p);
    mu = 1.0;
    bool changed = strat.update(mu, 1e-10, 1e-10, 0.5, -1.0, true,
                                1e-3, 0.0, 1.0);
    if (changed) {
        FAIL("a tiny FTB limit should hold the barrier parameter");
        return;
    }
    PASS();
}
...
int main() {
    ...
    test_at_minimum();
    test_normal_path();
    test_ftb_and_accepted_step_are_separate();
