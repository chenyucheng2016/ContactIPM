...
int main() {
    ...
    ContactIPMParams pp;
    pp.mu_init  = 1.0;  // Start from moderate barrier, not 10.0
    pp.max_iters = 300;
    // Matched tolerances for fair comparison vs acados.
    pp.mu_min = 1e-4;
    pp.tol_primal = 1e-4; pp.tol_compl = 1e-4; pp.tol_ineq = 1e-4; pp.tol_stat = 0.1;
    ...
    pp.exact_hessian = true;          // full Lagrangian Hessian (exact Newton)
    pp.verbosity = 2;
    solver.configure(pp);
    ...
    for (int ii = 0; ii < NTIMINGS; ++ii) {
        ...
        ContactIPMParams quiet = pp; quiet.verbosity = 0;
