...
public:
    ...
    bool update(double& mu, double primal_inf, double compl_inf, double sigma,
                ...
                double alpha_ftb = 1.0, double max_g_pos = 0.0,
        ...
        if (alpha_accepted < p_.alpha_tiny_thresh)
            tiny_alpha_count_++;
        else
            tiny_alpha_count_ = 0;

        // Count consecutive very-tiny alpha steps (restoration trigger).
        if (alpha_accepted < p_.restoration_alpha_thresh)
        ...
        if (phase_ == Phase::INFEASIBILITY && max_g_pos > p_.epsilon_g
            && restoration_alpha_count_ >= p_.restoration_count_limit
            && !restoration_active_) {
        ...
        if (phase_ == Phase::INFEASIBILITY && !force_by_stall && !force
            ...
            if (p_.verbosity >= 2)
                printf("  [barrier: PHASE-A HOLD max_g+=%.3e>%.3e"
                       " mu=%.3e alpha_p=%.4f]\n",
                       max_g_pos, p_.epsilon_g, mu, alpha_accepted);
            return false;
        }
        ...
        if (alpha_ftb < p_.ftb_hold_thresh && !force_by_stall) {
            // Alpha too small — hold μ to prevent worsening
            if (p_.verbosity >= 2)
                printf("  [barrier: FTB-HOLD alpha_p=%.4f<%.2f"
                       " mu=%.3e]\n",
                       alpha_ftb, p_.ftb_hold_thresh, mu);
            return false;
        }

        if (alpha_ftb < p_.ftb_slow_thresh) {
            sigma_exp = 0.8;
            difficulty = "ftb-slow";
        ...
        // At the barrier floor there is no update. Reporting a change here
        // made the solver clear its filter on every subsequent iteration.
        if (mu_new >= mu * (1.0 - 1e-14))
            return false;
