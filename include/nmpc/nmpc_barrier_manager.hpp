...
public:
    ...
    //
    //  Judges if barrier subproblem is solved: E_μ ≤ κ·μ
    ...
    bool update(double& mu, double primal_inf, double compl_inf, double sigma) {
        ...
        bool solved = (E_mu <= p_.kappa_eps * mu);
        bool force  = (iters_at_mu_ >= p_.max_same_mu);

        if (!solved && !force) {
            // Barrier subproblem not yet solved — hold μ fixed
            if (p_.verbosity >= 3)
                printf("  [barrier: HOLD E_mu=%.2e > k*mu=%.2e, iters=%d/%d]\n",
                       E_mu, p_.kappa_eps * mu, iters_at_mu_, p_.max_same_mu);
            return false;
        }
        ...
        if (p_.verbosity >= 2)
            printf("  [barrier: %s E_mu=%.2e, k*mu=%.2e, sig*mu=%.2e, kap*mu=%.2e -> mu=%.3e]\n",
                   solved ? "SOLVED" : "FORCE", E_mu, p_.kappa_eps * mu,
                   mu_adaptive, mu_monotone, mu);
        return true;
    }
