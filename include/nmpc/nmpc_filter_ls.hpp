...
struct LSResult {
    ...
    int      candidate;  // accepted trial candidate (0 = ordinary Newton trial)
    bool     soc_used;   // was SOC attempted?
    int      soc_iters;  // number of SOC steps used
    // Accepted trial data. Keeping these in the result avoids evaluating the
    // nonlinear model a second time merely to assess model agreement.
    double   theta0;
    double   theta_trial;
    double   phi0;
    double   phi_trial;
...
public:
    ...
    // A nonlinear solver may expose several retractions of the same tangent
    // step. Candidate 0 is always the ordinary Newton trial. Every candidate
    // is subjected to exactly the same filter and Armijo acceptance tests.
    virtual int trial_candidate_count() const { return 1; }
    virtual bool evaluate_candidate(int candidate, double alpha,
    ...
    // Restore the original search direction after an SOC candidate is
    // rejected and ordinary backtracking resumes.
    virtual void reject_soc() {}
...
inline bool bi_objective_improvement(double theta_trial, double theta0,
    ...
    if (theta0 <= theta_min &&
        theta_trial <= std::max(theta_min, 10.0 * theta0) &&
        phi_trial <= phi0 + gamma_phi) return true;
    return false;
}
...
public:
    ...
    LSResult search(Eval& eval, double alpha_max) {
        ...
        result.theta0 = result.theta_trial = 0.0;
        ...
        for (int l = 0; l < 50; ++l) {  // safety cap on backtrack count
            ...
            bool theta_control = theta_trial
                <= std::max(params_.theta_min, 10.0 * theta0);
            ...
            int accepted_candidate = accepted ? 0 : -1;
            double accepted_theta = theta_trial;
            double accepted_phi = phi_trial;

            // Alternative retractions use the same tangent derivative and the
            // same filter tests. Among admissible candidates, select by the
            // active filter phase: feasibility first away from the feasible
            ...
            for (int candidate = 1;
                ...
                bool in_filter_c = filter_.contains(theta_c, phi_c,
                                                     params_.gamma_theta,
                                                     params_.gamma_phi);
                bool armijo_c = armijo_condition(phi_c, phi0, Dphi, alpha,
                                                  params_.eta_phi);
                bool bi_obj_c = bi_objective_improvement(theta_c, theta0,
                                                          phi_c, phi0,
                                                          params_.gamma_theta,
                                                          params_.gamma_phi,
                                                          params_.theta_min);
                bool theta_control_c = theta_c
                    <= std::max(params_.theta_min, 10.0 * theta0);
                bool accepted_c = !in_filter_c
                    && (feas_case ? (armijo_c && theta_control_c) : bi_obj_c);

                if (v >= 1) {
                    printf("    [ls:%d.%d] a=%.3e theta=%.4e phi=%.4e"
                           " | filt=%d sw=%d arm=%d bi=%d case=%s\n",
                           l, candidate, alpha, theta_c, phi_c,
                           in_filter_c, sw_cond, armijo_c, bi_obj_c,
                           feas_case ? "OPT" : "FEAS");
                }

                const double theta_compare_tol =
                    1e-10 * std::max(1.0, std::fabs(accepted_theta));
                const double phi_compare_tol =
                    1e-10 * std::max(1.0, std::fabs(accepted_phi));
                bool better_selected = accepted_candidate < 0;
                if (accepted_candidate >= 0 && !feas_case) {
                    better_selected =
                        theta_c < accepted_theta - theta_compare_tol ||
                        (std::fabs(theta_c - accepted_theta)
                             <= theta_compare_tol &&
                         phi_c < accepted_phi - phi_compare_tol);
                } else if (accepted_candidate >= 0) {
                    better_selected =
                        phi_c < accepted_phi - phi_compare_tol ||
                        (std::fabs(phi_c - accepted_phi) <= phi_compare_tol &&
                         theta_c < accepted_theta - theta_compare_tol);
                }
                if (accepted_c && better_selected) {
                    accepted_candidate = candidate;
                    accepted_theta = theta_c;
                    accepted_phi = phi_c;
                }
            }

            accepted = accepted_candidate >= 0;
            if (accepted) {
                theta_trial = accepted_theta;
                phi_trial = accepted_phi;
                if (!feas_case
                    || theta0 > params_.theta_min || theta_trial > 1e-8)
                    filter_.add(theta_trial, phi_trial,
                                params_.gamma_theta, params_.gamma_phi);
                result.status   = LSStatus::ACCEPTED;
                result.alpha    = alpha;
                result.ls_iters = l + 1;
                result.candidate = accepted_candidate;
                result.theta_trial = theta_trial;
                result.phi_trial   = phi_trial;
                if (v >= 1)
                    printf("    [ls:%d.%d] ACCEPT a=%.4f case=%s\n",
                           l, accepted_candidate, alpha,
                           feas_case ? "OPT(armijo)" : "FEAS(bi-obj)");
                return result;
            }
            ...
            if (params_.soc_max > 0 && trigger_soc && l == 0
                && theta_trial >= theta0) {
                ...
                for (int p = 0; p < params_.soc_max; ++p) {
                    ...
                    if (!soc_in_filter) {
                        ...
                        if (soc_accepted) {
                            ...
                            result.ls_iters = l + 1;
                            result.candidate = 0;
                            result.theta_trial = theta_soc;
                            result.phi_trial   = phi_soc;
                            return result;
                        }
                    }
