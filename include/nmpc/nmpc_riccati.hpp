                                ...
                                double inertia_min_pivot = 0.0)
    {
        ...
        for (int k = N - 1; k >= 0; --k) {
            ...
            for (int attempt = 0; attempt < 6; ++attempt) {
                ...
                double d_min = 0.0;
                double d_max = 0.0;
                bool ldlt_ok = ws.S.ldlt_factorize(1e-14, &d_min);
                // Compute max pivot (always, for the inertia ratio check).
                for (int i = 0; i < NU; ++i)
                    if (ws.S(i, i) > d_max) d_max = ws.S(i, i);
                ...
                // LDLT succeeded. Check pivot ratio: a small positive pivot
                // silently poisons the Riccati recursion (P_k blowup) even
                // though the factorization nominally succeeded. Require
                //   d_min >= inertia_min_pivot * d_max.
                // Disabled (inertia_min_pivot==0) by default — reg_max alone
                // catches the catastrophic case.
                double pivot_floor = inertia_min_pivot * d_max;
                if (pivot_floor <= 0.0 || d_min >= pivot_floor) {
                    factored = true;
                    inertia_ok = true;
                    break;
                }
                // Small positive pivot: bump reg and retry.
